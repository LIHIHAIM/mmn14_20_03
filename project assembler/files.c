...
boolean buildOutputFiles(char *fileName){
     char *tempFileName = calloc(strlen(fileName) + MAX_SUFIX_LEN + 1, sizeof(char));
     FILE *fd = fopen(addSufix(tempFileName, ".ob", fileName), "w");
     if(!fd || !tempFileName)
          return FALSE;
     ...
     if(entryExist()){
          FILE *fd = fopen((addSufix(tempFileName, ".ent", fileName)), "w");
          if(!fd)
               return FALSE;
          pushEntryToFile(fd);
          fclose(fd);
     ...
     if(externalExist()){
          FILE *fd = fopen((addSufix(tempFileName, ".ext", fileName)), "w");
          if(!fd)
               return FALSE;
          pushExternToFile(fd);
          fclose(fd);
