...
boolean pushExtern(char *line, int *lInd, int lineCnt)
{
    ...
    if(label == NULL)
        ...
        printf("error [line %d]: after .extern directive must come an operand\n", lineCnt);
        return FALSE;
    }
...
{   
    ...
    if (isIlegalName(name, lineCnt) || wasDefined(name, lineCnt, TRUE, 1))
        return FALSE;
    tabInd = symSize-1;
    symTab[tabInd].symbol = calloc(strlen(name) + 1, sizeof(char));
    symTab[tabInd].attribute1 = calloc(strlen(attrib) + 1, sizeof(char));
...
boolean addToExtTab(char *name, int add){
    if (extSize == 0){
        externals = calloc(1, sizeof(extTable));
        extSize++;
    }
    else
    ...
    if (!isAlloc(externals))
        return ERROR;
    externals[extSize-1].name = calloc(strlen(name), sizeof(char));
...
static boolean addAttribToSymTab(char *sym, char *attrib)
{
    ...
    for (i = 0; i < symSize; i++)
    {
        ...
        {
            symTab[i].attribute2 = calloc(strlen(attrib) + 1, sizeof(char));
