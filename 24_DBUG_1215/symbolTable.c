...
boolean pushExtern(char *line, int *lInd, int lineCnt, boolean allocate){
    char *label;
    boolean added;

    jumpSpaces(line, lInd);
    label = readWord(line, lInd);
    if (!isValidLabel(label, lineCnt, FALSE) || isIlegalName(label, lineCnt) || wasDefined(label, lineCnt, TRUE, 1)){
        free(label);
        return FALSE;
    }
    if(allocate && !(added = addToSymTab(label, "external", lineCnt))){
        free(label);
        return FALSE;
    }
    if (added == ERROR){
