...
static word *scanInstParams(char *line, int *lInd, int lineCnt){
    ...
    jumpSpaces(line, lInd);
    while ((tempOperand = readWord(line, lInd)) != NULL){
    ...
    else if (tempOperand == NULL){ /* could not read the last operand or there are more than 2 operands */
        freeArr(operands, MAX_WORDS);
        free(memory);
        free(error);
        return NULL;
