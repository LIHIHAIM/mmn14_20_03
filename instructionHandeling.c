...
word getDecWord(char *toWord, int lineCnt)
{
    ...
    for (; i < strlen(temp); i++)
    {
        curr = temp[i];
        if(curr == '.'){
            printf("error [line %d]: In the immediate addressing method can be represented only decimal numbers (not valid : %s)\n", lineCnt, toWord);
            isDec = FALSE;
        }
        if (!isdigit(curr) && curr != '.')
        {
            printf("error [line %d]: In the immediate addressing method can be writen only numericl characters (not valid : %s)\n", lineCnt, toWord);
...
static word *scanInstParams(char *line, int *lInd, int lineCnt){
    ...
    for (; opAmount <= MAX_WORDS; opAmount++){
        if((line[*lInd] == '#' || line[*lInd] == '%') && isspace(line[*lInd+1])){
            printf("error [line %d]: After the immidiate or reltive addresting method indicators must come an operand (with no spaces between)\n", lineCnt);
            freeArr(operands, opAmount);
            return error;
        }

        if ((operands[opAmount] = readWord(line, lInd)) == NULL){
            freeArr(operands, opAmount);
        ...
        else if(strcmp(operands[opAmount], "\0") == 0)
            break;
        if (!isThereComma(line, lInd, lineCnt)){
            if(!isBlank(line, *lInd))
                isValidOpsAmount(operationInd, opAmount + 1, lineCnt);
            else
                isValidOpsAmount(operationInd, opAmount, lineCnt);
            freeArr(operands, opAmount);
            return error;
        }
        if (opAmount == MAX_WORDS && !isBlank(line, *lInd)){
            if (isThereComma(line, lInd, lineCnt))
                printf("error [line %d]: To an instruction command can be added at most a source and a target operand\n", lineCnt);
            else
                isValidOpsAmount(operationInd, opAmount, lineCnt);
            freeArr(operands, opAmount);
            return error;
    ...
    if (!isValidOpsAmount(operationInd, opAmount - 1, lineCnt)){
        freeArr(operands, opAmount);
        return error;
    }

    memory = calloc(MAX_WORDS, sizeof(word));
    if(!isAlloc(memory)){
        free(error);
        freeArr(operands, opAmount);
        return NULL;
    ...
    if (opAmount-1 < 2){ /* if operand amount is 0 or 1 */
...
static boolean isValidSAM(int operandInd, int AM, int lineCnt)
{
    ...
    printf("error [line %d]: The addressing method for the source operand (operation: \"%s\") is not valid\n", lineCnt, instsList[operandInd].name);
    return FALSE;
}
...
static boolean isValidTAM(int operandInd, int AM, int lineCnt)
{
    ...
    printf("error [line %d]: The addressing method for the target operand (operatioin: \"%s\") is not valid\n",  lineCnt, instsList[operandInd].name);
    return FALSE;
}
...
static int getOperationIndex(char *operation, int lineCnt)
{
    ...
    printf("error [line %d]: Invalid operation name \"%s\" (no such instruction command)\n", lineCnt, operation);
    return -1;
}
...
static int getAddressingMethod(char *operand, int lineCnt)
{
    ...
    case '%':
        if (strlen(operand) > 1)
            return relative;
        printf("error [line %d]: After the addressing method signed - modulu, must come a label (with no spaces between)\n", lineCnt);
        break;
    case 'r':
    {
        int i;
    ...
    for (i = 0; i < INSTRUCTIONS; i++){
        if (i < REGISTERS){
            if (label[0] == 'r' && i == atoi(label+1)){
                printf("error [line %d]: Label (\"%s\") should not be named as a register\n",lineCnt, label);
                return TRUE;
            }
        }
        if(i < DIRECTIVES){
            if(strcmp(directives[i], label) == 0){
                printf("error [line %d]: Label (\"%s\") should not be named as a directive\n", lineCnt, label);
                return TRUE;
            }
        }
        if (strcmp(instsList[i].name, label) == 0){
            printf("error [line %d]: Label (\"%s\") should not be named as an instruction\n", lineCnt, label);
            return TRUE;
        }
