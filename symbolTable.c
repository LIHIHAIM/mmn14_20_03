...
static extTable *externals = NULL;
static int extSize = 0;
...
boolean pushEntry(char *line, int* lInd, int lineCnt){
    ...
    if(strcmp(operation, ".entry") == 0){
        ...
        if(definedAs(operand, "external")){
            printf("error [line %d]: Operand cannot be named \".extern\" and \".entry\" at the same time\n", lineCnt);
            return FALSE;
        }
...
{   
    ...
    if (symSize == 0){
        ...
        if(valid){
            ...
            for(i =0; i < 1000; i++);
                printf(" \r");
            symTab = realloc(symTab, ((symSize+1) * sizeof(symbolTable)));
            for(i =0; i < 1000; i++);
                printf(" \r");
            printf("realloced\n");
            symSize += 1;
