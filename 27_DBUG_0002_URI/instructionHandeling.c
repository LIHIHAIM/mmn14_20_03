...
boolean pushInstSecond(char *line, int *lInd, int lineCnt){
    ...
    case relative:
        ...
        if (wasDefined(operand, lineCnt, FALSE, 2)){
...
static word getDecWord(char *toWord, int lineCnt){
    int i = 0, decimal;
...
static word *scanInstParams(char *line, int *lInd, int lineCnt){
    ...
    char **operands = calloc(MAX_WORDS, sizeof(char *)), *tempOperand;
    ...
    jumpSpaces(line, lInd);
    while ((tempOperand = readWord(line, lInd)) != NULL && strcmp(tempOperand, "\0") != 0){
        if(strcmp(tempOperand, "%") == 0 || strcmp(tempOperand, "#") == 0){
            printf("error [line %d]: after an addressing method sign must come an operand (without spaceses between)\n", lineCnt);
            free(memory);
            freeArr(operands, MAX_WORDS); /* new */
            return error;
        }        
    ...
    else if (operands[opAmnt-1] == NULL || strcmp(operands[opAmnt-1], "\0") == 0){ /* could not read the last operand or there are more than 2 operands */
        freeArr(operands, MAX_WORDS);
        free(memory);
        free(error); /* new */
        return NULL;
